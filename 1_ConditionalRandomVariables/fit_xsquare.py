...
W_mu = tf.Variable(tf.random_normal([n_hidden, 1], stddev=1/np.sqrt(n_hidden)), name='W_mu')
b_mu = tf.Variable(tf.zeros([1]), name='b_mu')
mu      = tf.add(tf.matmul(h1, W_mu), b_mu)

# Softplus mapping to sigma
W_sigma = tf.Variable(tf.random_normal([n_hidden, 1], stddev=1/np.sqrt(n_hidden)), name='W_sigma')
...
for epoch in range(epochs):
    ...
    if epoch % 50 == 0: # Fit
        ...
        plt.cla()
        plt.scatter(x_test[idx,0], y_test[idx,0], c='r', marker='.')
